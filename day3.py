...
def parse_instructions(instruction_string: str):
    wires: List[str] = instruction_string.split('\n')

    def parse_line(line: str):
        raw_instructions: List[str] = line.split(',')
        return [(raw_instruction[0], int(str(raw_instruction[1:]))) for raw_instruction in raw_instructions]

    return (parse_line(wires[0]), parse_line(wires[1]))


def find_closest_intersection(line_1, line_2):
    # X=20000
    # Y=20000
    # # Right = Positive X
    # # Up = Positive Y
    # # Create grid of 5Kx5K, and mark the middle as the center
    # print('Creating grid')
    # grid = [[False for y in range(Y)] for x in range(X)]
    # print('Grid created')

    # # Returns distance from 0,0 if point was previously traversed
    # def set_grid_traversed(x, y):
    #     # print('Traversing', x, y)
    #     internal_x = int(x+(X/2))
    #     internal_y = int(y+(Y/2))
    #     if internal_x >= X:
    #         print('x too large', x)
    #         return 0
    #     if internal_y >= Y:
    #         print('y too large', y)
    #         return 0
    #     current_value = grid[internal_x][internal_y]
    #     grid[internal_x][internal_y] = True
    #     if current_value:
    #         return abs(x) + abs(y)
    #     return 0
    hit_map = {}
    ...
    for (direction, distance) in line_1:
        if direction == 'U':
            for i in range(distance):
                set_grid_traversed(current_x, current_y+(i+1))
            current_y += distance
        if direction == 'D':
            for i in range(distance):
                set_grid_traversed(current_x, current_y-(i+1))
        ...
        if direction == 'R':
            for i in range(distance):
                set_grid_traversed(current_x+(i+1), current_y)
            current_x += distance 
        if direction == 'L':
            for i in range(distance):
                set_grid_traversed(current_x-(i+1), current_y)
    ...
    for (direction, distance) in line_2:
        if direction == 'U':
            for i in range(distance):
                intersection_distance = get_grid_traversed(current_x, current_y+(i+1))
                if intersection_distance > 0:
                    intersections.append((current_x, current_y+(i+1), intersection_distance))
                    if intersection_distance < shortest_distance:
                        shortest_distance = intersection_distance
            current_y += distance
        if direction == 'D':
            for i in range(distance):
                intersection_distance = get_grid_traversed(current_x, current_y-(i+1))
        ...
        if direction == 'R':
            for i in range(distance):
                intersection_distance = get_grid_traversed(current_x+(i+1), current_y)
                if intersection_distance > 0:
                    intersections.append((current_x+(i+1), current_y, intersection_distance))
                    if intersection_distance < shortest_distance:
                        shortest_distance = intersection_distance
            current_x += distance 
        if direction == 'L':
...
if __name__ == "__main__":
    (line_1, line_2) = parse_instructions(input_file)
