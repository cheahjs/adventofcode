...
def main(input_file):
    ...
    while q:
        ...
        # visit all possible neighbours, which is straight, turn left or right
        new_dirs = [direction, (direction - 1) % 4, (direction + 1) % 4]
        for new_dir in new_dirs:
            new_x = x + d[new_dir][0]
            new_y = y + d[new_dir][1]
            new_steps = steps + 1 if new_dir == direction else 1
            # ignore if we have taken more than 3 steps forward or out of bounds
            if new_steps > 3 or new_x >= max_x or new_x < 0 or new_y >= max_y or new_y < 0:
                continue
