        ...
        for x in range(len(board[y])):
            ...
            if current_char.isdigit():
                ...
                for test in adj_tests:  
                    ...
                    if test_char == '*':
                        if adjacent_gear and adjacent_gear != (x_test, y_test):
                            print('ERROR: MULTIPLE adjacent gears')
                        adjacent_gear = (x_test, y_test)
            # We've hit the end of a digit
            else:
                if current_digit != 0 and adjacent_gear:
                    gears[adjacent_gear].append(current_digit)
                current_digit = 0
                adjacent_gear = None
